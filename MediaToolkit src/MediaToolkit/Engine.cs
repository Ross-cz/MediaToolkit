namespace MediaToolkit
{
	...
	public class Engine : EngineBase
	{
		...
		public event EventHandler<MediaFileEventArgs> MediaFileEvent;

		public void CustomCommand(string ffmpegCommand, string workingFolder = null)
		{
		...
		private void OnMediaFileData(MediaFileEventArgs e)
		{
			MediaFileEvent?.Invoke(this, e);
		}
		...
		private void StartFFmpegProcess(EngineParameters engineParameters)
		{
			...
			processStartInfo.WorkingDirectory = engineParameters.WorkingFolder;
			...
			using (this.FFmpegProcess = Process.Start(processStartInfo))
			{
				...
				this.FFmpegProcess.ErrorDataReceived += (sender, received) =>
				{
									...
									try
					{
						...
						MediaFileEventArgs mediaFileEvent;

						// there is input first inside log, so after that mark it as output on the first line start "Output" occurrance
						if (received.Data?.StartsWith("Output ") == true)
						{
							isOutput = true;
						}

						if (RegexEngine.IsMediaFileData(isOutput, received.Data, out mediaFileEvent))
						{
							mediaFileEvent.MediaFile.Metadata.Duration = totalMediaDuration;
							OnMediaFileData(mediaFileEvent);
						}
						else if (RegexEngine.IsProgressData(received.Data, out progressEvent))
						{
